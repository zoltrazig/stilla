...
//! source order) with the `Span` of the offending token, and `diag` names
//! the first; node spans let later stages (type checking, code
//! generation) point at the exact source text. Parsing recovers from
//! syntax errors at statement and module-item boundaries (panic mode),
//! so a whole file's independent errors surface in one run.
...
pub const Parser = struct {
    ...
    /// Every syntax error, in source order (arena-owned). Present even
    /// after a lexer failure (the lexer's diagnostics are copied here).
    ...
    depth: u32 = 0,
    /// Brace nesting depth, maintained by every token-consuming helper.
    /// Statement-level recovery (`recoverStmt`) uses it to tell a nested
    /// construct's closing `}` from the enclosing block's, so an error
    /// inside an `if`/block expression cannot consume the wrong brace.
    brace_depth: u32 = 0,
    ...
    /// diagnostic messages, on error) reference the arena, so both are valid
    /// only until `deinit`. The caller keeps `source` (and its text) alive
    /// for the program's lifetime. On syntax errors the parse recovers at
    /// item/statement boundaries and returns `error.Syntax` with every
    ...
    pub fn parse(self: *Parser, source: *const ast.Source) ParseError!ast.Program {
        ...
        self.tokens = lexer.tokenize() catch |err| {
            // A lexical error: the token stream is unreliable, so parsing
            // stops — but every lexical diagnostic (not just the first)
            // carries over to the parser's collection.
            self.diags = lexer.diags;
            self.diag = lexer.diag;
            return err;
        };
        self.pos = 0;
        const program = ast.Program{ .items = try self.parseModuleItems() };
        // Panic-mode recovery may have produced a partial AST; a file
        // with any syntax error still fails as a whole (the diagnostics
        // are all collected), matching the pre-recovery contract.
    ...
    pub fn parseModuleItems(self: *Parser) ![]ast.ModuleItem {
        ...
        while (!self.at(.eof)) {
            const item = self.parseModuleItem() catch |err| switch (err) {
                error.Syntax => blk: {
                    // Panic-mode recovery at the module-item boundary: the
                    // malformed item's diagnostic is already recorded;
                    // drop it and resynchronize at the next item start.
                    self.syncToItemBoundary();
                    break :blk null;
                },
                error.OutOfMemory => return error.OutOfMemory,
            };
            if (item) |it| try items.append(self.arena.allocator(), it);
        }
        return self.arena.allocator().dupe(ast.ModuleItem, items.items);
    }

    /// Panic-mode recovery at module-item level: skip tokens until the
    /// next token that can start a module item (or EOF). Module items
    /// start with keywords that never appear inside another item (a
    /// `fn (` lambda is not treated as an item start), so the sync is
    /// safe without brace tracking.
    fn syncToItemBoundary(self: *Parser) void {
        while (!self.at(.eof)) {
            switch (self.cur().kind) {
                .kw_const, .kw_type, .kw_struct, .kw_union, .kw_opaque, .kw_using => return,
                // A `fn` item has a name; a `fn (` is a lambda inside an
                // expression, not an item start.
                .kw_fn => {
                    if (self.peekKind() != .lparen) return;
                    self.pos += 1;
                },
                else => self.pos += 1,
            }
    ...
    /// The kind of the token after the current one (`.eof` at the end).
    fn peekKind(self: *Parser) lex.TokenKind {
        if (self.pos + 1 >= self.tokens.len) return .eof;
        return self.tokens[self.pos + 1].kind;
    }

    /// Panic-mode recovery at statement level (used by the block loop):
    /// skip tokens until the next statement boundary at this block's own
    /// brace depth. `;` is consumed (it terminates the abandoned
    /// statement); a `}` that closes this block (`brace_depth ==
    /// loop_depth`) is left for the block loop, while a nested
    /// construct's closing `}` is consumed so an error inside an
    /// `if`/block expression cannot unbalance the enclosing block. Guar-
    /// antees progress: every iteration consumes a token or stops at EOF.
    pub fn recoverStmt(self: *Parser, loop_depth: u32) void {
        while (!self.at(.eof)) {
            const depth = self.brace_depth;
            switch (self.cur().kind) {
                .lbrace => {
                    _ = self.advance();
                },
                .rbrace => {
                    if (depth <= loop_depth) return;
                    _ = self.advance(); // closes a nested construct
                },
                .semicolon => {
                    _ = self.advance();
                    if (depth == loop_depth) return;
                },
                .kw_let, .kw_drop, .kw_using => {
                    if (depth == loop_depth) return;
                    _ = self.advance();
                },
                else => _ = self.advance(),
    ...
    pub fn fail(self: *Parser, span: ast.Span, comptime fmt: []const u8, args: anytype) error{Syntax} {
        ...
        self.diags.append(self.arena.allocator(), d) catch {};
        return error.Syntax;
    }
