...
test "moduleinfo resolves the stdbundle standard-library modules" {
    ...
    for (math.values) |*vm| {
        if (!math.isIntrinsic(vm)) all_intrinsic = false;
    }
    try testing.expect(all_intrinsic);
}
...
test "moduleinfo isHostBinding distinguishes host and source modules" {
    ...
    try testing.expect(!app.isHostBinding("main"));
    // Bundle bodyless members are intrinsics (Intrinsics §2), never host
    // bindings: `builtin.print` is bodyless but absent from
    // `host_bindings` — that set holds only non-bundle declarations.
    try testing.expect(!builtin.isHostBinding("print"));
    try testing.expectEqual(@as(usize, 0), builtin.host_bindings.len);
}

test "moduleinfo marks only the embedded std bundle with bundle_origin" {
    // Origin (Intrinsics §2): the embedded `std/` bundle is the only
    // source of intrinsic identity. Caller-supplied stdlib extensions,
    // user source modules, and host modules are never bundle-origin —
    // same spelling changes nothing.
    var arena = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();
    const arena_alloc = arena.allocator();
    ...
    var std_map = std.StringHashMapUnmanaged([]const u8).empty;
    defer std_map.deinit(testing.allocator);
    try std_map.put(testing.allocator, "extra", "fn print(x: str) -> void;");
    sources.standard_library = std_map;
    var host_map = std.StringHashMapUnmanaged(void).empty;
    defer host_map.deinit(testing.allocator);
    try host_map.put(testing.allocator, "os", {});
    ...
    var source_map = std.StringHashMapUnmanaged([]const u8).empty;
    defer source_map.deinit(testing.allocator);
    try source_map.put(testing.allocator, "app", "const math = import(\"math\");\nconst extra = import(\"extra\");\nconst os = import(\"os\");\nfn main() -> void {}");
    sources.source = source_map;

    var b = moduleinfo.Builder.init(arena_alloc, sources);
    const graph = try b.build("app");

    try testing.expect(graph.module("math").?.bundle_origin);
    try testing.expect(!graph.module("extra").?.bundle_origin);
    try testing.expect(!graph.module("os").?.bundle_origin);
    try testing.expect(!graph.module("app").?.bundle_origin);
}

test "moduleinfo classifies value members by origin: intrinsic vs host binding" {
    // Three-state classification (Intrinsics §2): bundle bodyless
    // declarations are intrinsics; non-bundle bodyless declarations are
    // host bindings even when they spell like a bundle member (source
    // spoofing); everything with a definition is ordinary.
    var t = try buildGraph("app", &.{
        .{
            "app",
            \\const math = import("math");
            \\const lst = import("list");
            \\fn print(x: str) -> void;
            \\fn real_fn(x: int32) -> int32 { x }
            \\const host_const: int32;
            \\fn main() -> void {}
    ...
    const app = t.graph.module("app").?;
    const math = t.graph.module("math").?;
    const list = t.graph.module("list").?;
    // User-module spoof of `builtin.print`: bodyless, but not
    // bundle-origin — a host binding, not an intrinsic.
    try testing.expectEqual(moduleinfo.MemberClass.host_binding, app.valueMember("print").?.class);
    try testing.expectEqual(moduleinfo.MemberClass.host_binding, app.valueMember("host_const").?.class);
    try testing.expectEqual(moduleinfo.MemberClass.ordinary, app.valueMember("real_fn").?.class);
    try testing.expect(!app.isIntrinsic(app.valueMember("print").?));
    // Bundle bodyless members are intrinsics: math functions and consts.
    try testing.expectEqual(moduleinfo.MemberClass.intrinsic, math.valueMember("sqrt").?.class);
    try testing.expectEqual(moduleinfo.MemberClass.intrinsic, math.valueMember("pi").?.class);
    try testing.expect(math.isIntrinsic(math.valueMember("sqrt").?));
    try testing.expect(math.isIntrinsic(math.valueMember("pi").?));
    // Bundle members with a Stilla body are ordinary (StdLib §7–§8):
    // `list.is_empty` is defined in Stilla; `list.len`/`range` are the
    // bodyless intrinsics of the same module.
    try testing.expectEqual(moduleinfo.MemberClass.ordinary, list.valueMember("is_empty").?.class);
    try testing.expectEqual(moduleinfo.MemberClass.intrinsic, list.valueMember("len").?.class);
    try testing.expect(list.isIntrinsic(list.valueMember("len").?));
    try testing.expect(!list.isIntrinsic(list.valueMember("is_empty").?));
}
