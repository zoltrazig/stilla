...
pub fn annotateModule(ck: *checker.Checker, info: *ModuleInfo) CheckError!void {
    ...
    for (program.items) |*item| {
        annotateItem(ck, frame, item) catch |err| switch (err) {
            // Per-item continuation: one broken declaration does not
            // hide its siblings' errors (the diagnostic is recorded;
            // `names` for members are registered before the body is
            // checked, so later items resolve them).
            error.Diagnostic => {},
...
/// Annotate one module item (const initializer, function body, struct
/// drop hook). The frame is shared across items; every mutating helper
/// restores its state on error (`defer`), so a failed item leaves the
/// frame clean for its siblings.
fn annotateItem(ck: *checker.Checker, frame: *Frame, item: *ast.ModuleItem) CheckError!void {
    switch (item.*) {
        .const_def => |*c| {
            if (c.type_ != null) {
                const resolved = try ck.resolveTypeOf(frame.ma, frame.info, &c.type_.?);
                ...
                try frame.ma.names.put(ck.alloc(), c.name.text, &c.name);
            }
            if (c.init) |init| {
...
/// Numeric binary operands: infer the left operand, then treat its type as
/// the right operand's expected context. An integer literal on the right
/// then adapts to an integer left operand — `u - 1` with `u: uint32`, or
/// `d - 1` with `d: i64` — and a float literal to a float left operand
...
fn inferExprInner(frame: *Frame, e: *const ast.Expr) CheckError!?cfg.Type {
    ...
    switch (e.*) {
        .int => |*lit| {
            // Frozen rule (Core Types §16.3): an integer literal types
            // `int32` unless its uniquely-expected context is an integer
            // type, in which case it is typed at that width.
            if (frame.expect) |t| {
                if (t == .primitive and isIntWidth(t.primitive)) {
                    switch (t.primitive) {
                    ...
                    try frame.ck.annotation.int_widths.put(frame.ck.alloc(), lit, t.primitive);
                    return t;
                }
            }
        ...
        .cast => |*c| {
            // A cast operand carries its own type — the target type is
            // resolved on the cast — so it never inherits the surrounding
            // expected context: `3 as uint32` inside a `uint32` argument
            // must cast the int32 literal `3`, not widen it first.
            const old = frame.expect;
            frame.expect = null;
            defer frame.expect = old;
...
fn inferCall(frame: *Frame, c: *const ast.Call) CheckError!?cfg.Type {
    ...
    if (try inferCalleeDecl(frame, callee)) |target| {
        ...
        if (decl.type_params.len > 0) {
            // StdLib §1–§3 (Core §12.2): the array/hashmap members whose
            // type parameters occur only in the token or result type
            // carry no instantiation info in their arguments — the type
            // arguments must be written explicitly (`::[...]`), never
            // inferred. Origin-guarded: the requirement binds the
            // embedded bundle's interface, not a same-spelled user or
            // host module (Intrinsics §2).
...
/// parameters occur only in the token or result type (array §1–§2,
/// hashmap §3): `array.len/get/clone` carry `T` only inside `Array[T]`
/// and `hashmap.empty/get/contains/remove/len/clone` carry `K`/`V` only
/// inside `HashMap[K, V]`/`Option[V]`. A value of those named types
/// exposes no instantiation information, so the arguments must be
/// written explicitly (`::[...]`, Core §12.2/§12.3; StdLib: "their type
/// argument must be written explicitly"). `insert` is excluded — its
/// `K`/`V` are inferred from the key and value arguments (StdLib §3).
fn requiresExplicitTypeArgs(target: moduleinfo.PathTarget) bool {
    if (!target.module.bundle_origin) return false;
    const m = target.module.specifier;
    const n = target.vm.name.text;
    if (std.mem.eql(u8, m, "array")) {
        return std.mem.eql(u8, n, "len") or std.mem.eql(u8, n, "get") or std.mem.eql(u8, n, "clone");
    }
    if (std.mem.eql(u8, m, "hashmap")) {
        return std.mem.eql(u8, n, "empty") or std.mem.eql(u8, n, "get") or std.mem.eql(u8, n, "contains") or
            std.mem.eql(u8, n, "remove") or std.mem.eql(u8, n, "len") or std.mem.eql(u8, n, "clone");
    }
    return false;
}
