...
/// Origin-based member classification (Intrinsics §2): a declaration is
/// an intrinsic iff it is bodyless and its module is the embedded `std/`
/// bundle; bodyless declarations elsewhere are host bindings; anything
/// with a definition is ordinary.
fn memberClass(info: *const ModuleInfo, bodyless: bool) moduleinfo.MemberClass {
    if (!bodyless) return .ordinary;
    return if (info.bundle_origin) .intrinsic else .host_binding;
}
...
pub fn materialize(self: *Builder, raw: *RawModule) !void {
    ...
    var values = std.ArrayList(ValueMember).empty;
    var types = std.ArrayList(TypeMember).empty;
    var aliases = std.ArrayList(UsingAlias).empty;
    var host_bindings = std.ArrayList(HostBinding).empty;
    ...
    for (program.items) |*item| switch (item.*) {
        .func_def => |*f| {
            ...
            if (f.body == null and !info.bundle_origin) {
    ...
    for (raw.consts.items) |rc| {
        ...
        var member = ValueMember{
            ...
            .type_ = undefined,
            .decl = .{ .const_ = c },
            .host = c.init == null and !info.bundle_origin,
            .class = memberClass(info, c.init == null),
        };
    ...
    // order, Runtime §2.3).
    var imports = std.ArrayList(*ModuleInfo).empty;
