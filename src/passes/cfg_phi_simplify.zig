...
//! kept: it is unreachable or in the start block, and the AIR has no
...
//! (air.md §13). Allocations use the program's backing allocator (the
...
/// the start block, and the AIR has no undefined value to replace it with
...
/// hand-written AIR (`%a = phi [%b], [%a]`, `%b = phi [%a], [%b]`).
fn trivialSource(
