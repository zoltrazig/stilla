...
//! comparisons (`eq`/`ne`/`lt`/`le`/`gt`/`ge`), `not_`, and `type_is`.
//! Arithmetic (overflow, Runtime §7.2), `div`/`rem` (divisor zero), `cast`
//! (out-of-range float→int), and the reads/projections (bounds) all trap,
...
//! keep the round-trip contract (ir.md §13). Allocations use the program's
...
fn eliminate(
    ...
    // Availability per predecessor, in `b.preds` order (ir.md §4.3). The
