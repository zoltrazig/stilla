...
//!
//! The phase-1 builder and the phase-3 lowerer share these helpers: the
//! resolution core is written once against a `Resolve` view so both the
//! builder (while the graph is under construction) and the lowering (on the
//! finished graph) share the exact same behavior.
//!
...
fn resolveTypeNameInner(resolve: Resolve, from: *ModuleInfo, name: []const u8, owner: ?*?[]const u8) ?*TypeMember {
    if (std.mem.indexOfScalar(u8, name, '.')) |dot| {
        ...
        // A canonical `specifier.Name` reference (air.md §11): the head may
