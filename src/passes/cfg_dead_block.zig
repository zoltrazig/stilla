...
//! incoming-list contract (ir.md §4.3) requires one entry per predecessor,
...
//! defined (ir.md §13). An SSA use of a dead-block value can only live in
...
fn marks(f: *cfg.IrFunc, allocator: std.mem.Allocator) ![]bool {
    ...
    // have removed blocks, leaving id gaps (ir.md §13).
    var max_id: usize = 0;
...
/// predecessors in order, preserving the ir.md §4.3 contract.
