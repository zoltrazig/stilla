//! Standalone AIR text parser — air.md §9 (the `cfg` text form).
//!
...
//! on the module graph or the type checker; where air.md §11 names
//! frontend artifacts, `cfg.zig` substitutes AIR-native equivalents (see
...
/// The defining-op name map, generated from the op schema (air.md §5,
...
pub const Parser = struct {
    ...
    // Program-level type environment (air.md §9.1): the text form
    ...
    fn parseProgram(self: *Parser) ParseError!IrProgram {
        ...
        while (true) {
            ...
            m.* = .{
                ...
                // The text form carries no member declarations (air.md
    ...
    /// in the text (air.md §4.3: a loop back edge). Placeholder ids are
    ...
    fn parseDefiningInstr(self: *Parser) ParseError!void {
        ...
        // air.md §5.3 — only the atomic destructure ops define more than
        // one value).
        ...
        // The text form carries no syscall signature (air.md §10): `sig`
    ...
    fn parseIdentStatement(self: *Parser) ParseError!void {
        ...
        if (std.mem.eql(u8, text, "store_member")) {
            ...
            if (!std.mem.eql(u8, self.f_name, "init")) {
                return self.fail(t, "store_member is only legal inside @init (air.md §5.6)", .{});
            }
    ...
    /// The created state of a defined value (air.md §6.1): from the op
    ...
    /// (air.md §5.1), so their state is not computed here.
    ...
    fn parseOpOperands(self: *Parser, name: OpName) ParseError!Op {
        switch (name) {
            ...
            .abs => return .{ .abs = try self.parseOperand() },
            .clz => return .{ .clz = try self.parseOperand() },
            .popcount => return .{ .popcount = try self.parseOperand() },
            ...
            .min => return .{ .min = try self.parseBin() },
            .max => return .{ .max = try self.parseBin() },
            .shl => return .{ .shl = try self.parseBin() },
            .shr => return .{ .shr = try self.parseBin() },
            .bitand => return .{ .bitand = try self.parseBin() },
            .bitor => return .{ .bitor = try self.parseBin() },
            .bitxor => return .{ .bitxor = try self.parseBin() },
            ...
            // `select %cond, %a, %b` — three comma-separated operands,
            // the condition first.
            .select => {
                const cond = try self.parseOperand();
                try self.expectComma();
    ...
    /// True when an AIR literal token denotes a float32 constant: it
    ...
    /// Intern a named struct/union from AIR text: returns its stable `TypeId`,
    /// recording the written name on first sight (air.md §10; the text form
    ...
    fn parseType(self: *Parser) ParseError!Type {
        ...
        // `types[id]` round-trips (air.md §11). A trailing `[...]` is the
...
/// Parse one AIR text program for tests. `pub` because the printer's
...
test "cfg parses type_is tests and hostdata primitives" {
    ...
    // (Core §11.6.2); `hostdata` is a primitive type in the AIR text
    // (Core §11.7).
    var t = try parseText(
        \\module "app" {
