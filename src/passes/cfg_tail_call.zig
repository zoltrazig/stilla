//! Pass: tail-call optimization (optimizer.md, Pass 7; air.md §14.7). In: a
...
fn instrUses(instr: *const cfg.Instr, v: *const cfg.Value) bool {
    return switch (instr.op) {
        .neg, .abs, .clz, .popcount, .not_, .num_cast, .any_pack_copy, .any_pack_move, .any_unpack_copy, .any_unpack_move, .cleanup_arm, .cleanup_disarm, .cleanup_drop, .copy, .borrow, .move_, .tail, .unpack_struct, .unpack_tuple, .split_list, .read_tag, .read_payload, .drop_ => |x| x == v,
        .unpack_variant => |uv| uv.base == v,
        .borrow_variant => |bv| bv.base == v,
        .type_is => |x| x.value == v,
        .add, .sub, .mul, .div, .rem, .min, .max, .shl, .shr, .bitand, .bitor, .bitxor, .concat, .eq, .ne, .lt, .le, .gt, .ge => |x| x.a == v or x.b == v,
        .select => |x| x.cond == v or x.a == v or x.b == v,
...
fn rewriteFunc(f: *cfg.IrFunc, allocator: std.mem.Allocator) !void {
    var tails = try findTailCalls(f, allocator);
    defer allocator.free(tails);
    // v1 (Instruction Set §4): cleanup tokens are gone, so a maybe-owned local no
    // longer leaves a `cleanup_drop` behind in the join block — the join
    // is phi-only and the chain walk would accept shapes carrying
    // uniquely-owned loop state, which the phi-loop rewrite cannot express
    // soundly. Leave those for `emitTailCalls`' frame-reusing terminator.
    var kept = std.ArrayList(TailCall).empty;
    for (tails) |t| {
        var carries_unique = false;
        for (t.args) |a| {
            if (a.ownership == .unique and a.state == .owned) carries_unique = true;
        }
        if (!carries_unique) try kept.append(allocator, t);
    }
    tails = try kept.toOwnedSlice(allocator);
    ...
    // The text form's first block must have no predecessors (air.md §13),
...
fn rewriteInstr(instr: *cfg.Instr, renames: *const std.AutoHashMap(*cfg.Value, *cfg.Value)) void {
    switch (instr.op) {
        ...
        .select => |*x| {
            x.cond = renames.get(x.cond) orelse x.cond;
            x.a = renames.get(x.a) orelse x.a;
            x.b = renames.get(x.b) orelse x.b;
        },
...
// ---------------------------------------------------------------------------
// `tailcall` emission (air.md §14.7.1)
// ---------------------------------------------------------------------------
