//! Pass: AIR validity validator — the air.md §12 invariant set,
...
//!   air.md §6.4); a `borrowed` value is never consumed; `ret` never
...
fn validateFunc(program: *const cfg.IrProgram, f: *const cfg.IrFunc, allocator: std.mem.Allocator) !?[]const u8 {
    ...
    // (air.md §4.3, the one-incoming-per-pred contract): a branch to a
    ...
    // The entry block is dominated only by itself; every other block
    // starts dominated by everything and shrinks to the fixpoint (the
    // standard iterative dominator algorithm, as cfg_pre.dominators).
    // The previous bottom-up init (false everywhere except self and
    // predecessor-less blocks) converged to the *least* fixpoint, which
    // under-approximates true dominators on nested loops — an inlined
    // TCO loop inside an outer loop — falsely rejecting valid phi
    // incomings that the inliner's splice legitimately produces.
    {
        const ei = blkIndex(&index_of, f.entry) orelse unreachable;
        for (0..n) |j| dom[ei][j] = false;
        dom[ei][ei] = true;
    }
    var changed = true;
    while (changed) {
    ...
    // removes, and trap-terminated dead stubs, air.md §14.7): their *semantic*
    ...
    for (f.blocks, 0..) |b, i| {
        ...
        // its arriving edge's exit state (air.md §6.3: the phi transfers
        ...
        for (b.instrs) |instr| {
            ...
            for (phi.incoming) |inc| {
                ...
                // named predecessor block (air.md §13 SSA) — unless that
...
fn checkConsumedOperand(
    ...
    if (st.get(val)) |s| {
        switch (s) {
            ...
            .maybe => return msg(allocator, "function @{s}: '{s}' consumes maybe-unique value %{d} in block '{s}' (unusable after a conditional construct, air.md §6.4)", .{ f.name.text, cfg.opInfo(std.meta.activeTag(instr.op)).text, val.id, b.name }),
        }
    }
    return null;
}
...
fn checkAvailable(
    ...
    if (st.get(v)) |s| {
        switch (s) {
            ...
            .maybe => return msg(allocator, "function @{s}: {s} uses maybe-unique value %{d} in block '{s}' (unusable after a conditional construct, air.md §6.4)", .{ f.name.text, what, v.id, b.name }),
        }
    }
    return null;
}
...
fn checkInstr(
    ...
    for (ops) |v| {
        ...
        // Borrow provenance (air.md §6.5): a borrowed view's root must be
    ...
    switch (instr.op) {
        ...
        // `abs` wraps on int32_min (modulo 2³², never traps) and f32 is
        ...
        .num_cast => |v| {
            ...
            // (Core §16.3: every non-identity pair of the seven
        ...
        // min/max: total — the int32/uint32 forms compare as 32-bit
        ...
        .load_member => |x| {
            ...
            // The member table (air.md §7, §12): the member index must
            ...
            // modules without a member table (text-form AIR) skip the
            // check.
            ...
            switch (member.kind) {
                .const_slot => |slot| {
                    ...
                    // distinct index space from member indices (air.md
        ...
        .store_member => |x| {
            ...
            // slot must exist with a matching type (air.md §7, §12).
            if (!std.mem.eql(u8, f.name.text, "init")) {
                return msg(allocator, "function @{s}: store_member outside @init (air.md §5.6)", .{f.name.text});
            }
            if (f.module_spec) |spec| {
                if (cfg.moduleByName(program, spec)) |m| {
                    ...
                    // check is vacuous there — skip (air.md §10).
                    if (m.members != null) {
        ...
        .construct => |c| {
            ...
            // match the variant's payload arity (air.md §12). An untagged
            // construct (struct / tuple / list literal) may not target an
            // opaque type (air.md §12, opaque bases).
        ...
        .read_field => |x| {
            ...
            // `StructDecl` and the result is typed as that field (air.md
            // §12).
        ...
        .read_tuple => |x| {
            ...
            // result is typed as that element (air.md §12).
            switch (x.base.type_) {
                .tuple => |elems| {
        ...
        // types the destructure defines exactly the parts (air.md §12).
        .unpack_struct => |v| {
            if (v.state == .borrowed) {
        ...
        .read_tag, .read_payload => |v| {
            // Bases are unions (air.md §12); `read_tag` yields the
        ...
        .syscall => |s| {
            // The syscall's specialized signature (air.md §8.2, §12):
            ...
            if (s.sig) |sig| {
                ...
                for (s.args) |a| {
                    ...
                    if (p.mode == .move) {
                        ...
                        // can never be moved (Core §10.7, air.md §6.2).
                        if (a.state == .borrowed) {
...
/// exactly the union's variants (air.md §12).
fn checkTerminator(program: *const cfg.IrProgram, f: *const cfg.IrFunc, b: *const cfg.BasicBlock, dom: [][]bool, st: *StateMap, allocator: std.mem.Allocator) !?[]const u8 {
    switch (b.terminator) {
        ...
        .@"switch" => |sw| {
            ...
            // repeated tags (air.md §12). The union resolves through the
            // disc's defining `read_tag`; a switch whose disc does not
            // resolve (text-form AIR) skips the check.
            ...
            if (sw.arms.len != ru.u.variants.len) {
                return msg(allocator, "function @{s}: switch in block '{s}' has {d} arms for the {d} variants of '{s}' (air.md §12)", .{ f.name.text, b.name, sw.arms.len, ru.u.variants.len, ru.u.name });
            }
...
/// The value-operand count of an op (the 3-address arity, air.md §4.2).
fn valueOperandCount(op: cfg.Op) usize {
    return switch (op) {
        .const_, .module_ref, .fn_ref => 0,
        .neg, .abs, .clz, .popcount, .not_, .num_cast, .any_pack_copy, .any_pack_move, .any_unpack_copy, .any_unpack_move, .cleanup_arm, .cleanup_disarm, .cleanup_drop, .copy, .borrow, .move_, .tail, .unpack_struct, .unpack_tuple, .unpack_variant, .borrow_variant, .split_list, .read_tag, .read_payload, .drop_ => 1,
        .type_is => 1,
        .load_member => 1,
        .store_member => 1,
        .read_field, .read_tuple => 1,
        .read_index => 2,
        .add, .sub, .mul, .div, .rem, .min, .max, .shl, .shr, .bitand, .bitor, .bitxor, .concat, .eq, .ne, .lt, .le, .gt, .ge => 2,
        .select => 3,
...
/// The ultimate borrow root (air.md §6.5): follow a borrowed value's
/// origin chain to the root whose availability gates its uses — an owned
/// value or the `call` lifetime. A Copy root makes the availability
/// check vacuous (Copy values are never consumed).
...
/// air.md §6.5: every use of a borrowed value requires its ultimate root
...
/// §10.7); a Copy root makes the check vacuous. An owned unique root
/// must not be consumed on the reaching path.
fn checkBorrowRoot(
    f: *const cfg.IrFunc,
    b: *const cfg.BasicBlock,
    v: *const cfg.Value,
    ...
    switch (ultimateRoot(v)) {
        ...
        .value => |rv| {
            ...
            if (st.get(rv)) |s| {
                switch (s) {
                    .available => {},
                    .consumed => return msg(allocator, "function @{s}: use of borrowed value %{d} after its root %{d} was consumed in block '{s}' (air.md §6.5)", .{ f.name.text, v.id, rv.id, b.name }),
                    .maybe => return msg(allocator, "function @{s}: use of borrowed value %{d} whose root %{d} is maybe-unique in block '{s}' (air.md §6.5)", .{ f.name.text, v.id, rv.id, b.name }),
                }
            }
            return null;
        },
...
// ---------------------------------------------------------------------------
// air.md §12 type checks — struct / union / tuple / opaque layout
// ---------------------------------------------------------------------------

/// The declaration behind a named type (air.md §9.1): null for non-named
/// types and out-of-range ids. An `.unknown` row (text-form AIR — the
...
/// The union a `switch` dispatches on (air.md §12): the disc is a
/// `read_tag` result; its base resolves the union. Null when the disc is
/// not a `read_tag` of a concrete union (text-form AIR) — the
...
/// air.md §12 opaque bases: the excluded ops may not target an
...
// air.md §13 violations a lowering or optimizer bug would produce.
// ---------------------------------------------------------------------------
...
test "validator accepts a phi incoming defined by the outer loop header of a nested loop" {
    // An inner loop with an internal back-edge (inner ↔ inner_else) nested
    // inside an outer loop: the outer header's phi carries a self-incoming
    // from the outer back-edge (`cont`), whose definition is the header
    // itself. The header *does* dominate `cont` (every path to the
    // continuation passes through it), so the incoming is valid SSA. This
    // is the shape the inliner produces when a TCO'd callee is spliced
    // into a caller loop (fib_tail_call.st); the previous bottom-up
    // dominator init converged to the least fixpoint and under-approximated
    // the header's dominators, falsely rejecting it.
    const msg_ = try parseAndValidate(
        \\module "app" {
        \\    func @f() -> int32 {
        \\    entry:
        ...
        \\        j outer
        \\    outer:
        \\        %1: int32 = phi [%0, entry], [%1, cont]
        \\        %2: bool = const true
        \\        br %2 ? inner_entry : done
        \\    inner_entry:
        \\        j inner
        \\    inner:
        \\        %3: int32 = phi [%1, inner_entry], [%6, inner_else]
        \\        %4: bool = const false
        \\        br %4 ? inner_then : inner_else
        \\    inner_else:
        \\        %5: int32 = const 1
        \\        %6: int32 = add %3, %5
        \\        j inner
...
/// Parse text-form AIR, then install concrete type declarations over the
/// interned names (the text form carries no type declarations, air.md
...
test "validator accepts a union match over a concrete union (air.md §12)" {
...
test "validator rejects a read_field index out of range (air.md §12)" {
    const msg_ = try parseValidateWithDecls(
        \\module "use" {
        \\    func @f(p: Pair) -> int32 {
        \\    entry:
...
test "validator rejects a read_field whose result type mismatches the field (air.md §12)" {
    const msg_ = try parseValidateWithDecls(
        \\module "use" {
        \\    func @f(p: Pair) -> str {
        \\    entry:
...
test "validator rejects read_field and unpack_struct over a union base (air.md §12)" {
    const msg_ = try parseValidateWithDecls(
        \\module "use" {
...
test "validator rejects a construct with a mismatched payload arity (air.md §12)" {
    const msg_ = try parseValidateWithDecls(
        \\module "use" {
        \\    func @f() -> Result {
        \\    entry:
...
test "validator rejects unpack_variant and borrow_variant arity mismatches (air.md §12)" {
    const msg_ = try parseValidateWithDecls(
        \\module "use" {
...
test "validator rejects borrow_variant over a struct base (air.md §12)" {
    const msg_ = try parseValidateWithDecls(
        \\module "use" {
        \\    func @f(p: Pair) -> void {
        \\    entry:
...
test "validator rejects a non-exhaustive switch (air.md §12)" {
    const msg_ = try parseValidateWithDecls(
        \\module "use" {
        \\    func @msg(result: Result) -> str {
        \\    entry:
...
test "validator accepts a read_field over a generic struct instantiation (air.md §12)" {
    // The text form carries no type declarations or type arguments (air.md
