...
/// Build the program's type environment (ir.md §9.1): one `TypeDecl` per
...
/// One concrete `TypeDecl` from a module's type member (ir.md §9.1):
/// fields/variants resolved to IR-native types (a generic declaration's
...
/// Resolve a struct's field declarations to IR-native types (generic
...
/// Resolve a union's variant payload types to IR-native types (generic
/// declarations keep their type parameters as `Type.param`).
fn lowerVariants(self: *Lowerer, info: *moduleinfo.ModuleInfo, variants: []const ast.VariantDecl) LowerError![]cfg.VariantDecl {
    ...
    for (variants, 0..) |v, i| {
        var payloads = std.ArrayListUnmanaged(cfg.Type).empty;
