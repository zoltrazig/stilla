...
//! (air.md §6.4 "drop hooks" / §6.3, Runtime §6.2–§6.3). In: an optimized
...
//!   §6.6); the host owns the payload, the AIR cannot expand it;
...
//! unchanged — air.md §6.4, "no pass moves destruction earlier"):
//!
...
//!
//! The pass still resolves through the phase-1 module graph (the AIR's
...
//! validated by the frontend afterwards (air.md §13).
//!
...
//! to the function; values are renumbered at the end (air.md §4.1). The
//! `cleanup_arm` / `cleanup_disarm` / `cleanup_drop` family is never
//! touched: a cleanup token's payload liveness is runtime state
//! (air.md §6.4), and expansion into flag stores is a backend matter.
...
const Seq = struct {
    ...
    fn newBlock(seq: *Seq) !*cfg.BasicBlock {
        ...
        const out = try c.allocator.create(std.ArrayList(*cfg.Instr));
        out.* = .empty;
        seq.b = b;
        seq.out = out;
        seq.branching = false;
...
fn redirectSuccessors(c: *Ctx, b: *cfg.BasicBlock, old_term: cfg.Terminator, join: *cfg.BasicBlock) !void {
    ...
    const targets = switch (old_term) {
        ...
        .@"switch" => |s| blk: {
            var ts = std.ArrayList(*cfg.BasicBlock).empty;
...
fn emitUnionDrop(seq: *Seq, v: *cfg.Value, ref: DeclRef, args: []const cfg.Type, u: *const ast.UnionDef, span: ast.Span) error{OutOfMemory}!void {
    ...
    var finals = std.ArrayList(*cfg.BasicBlock).empty;
