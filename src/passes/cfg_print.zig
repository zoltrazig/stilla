//! Canonical AIR text printer — air.md §9 (the `cfg` text form).
//!
//! Serializes the in-memory CFG structures of `cfg.zig` back to the
//! canonical text form. Print order is the round-trip contract (air.md
...
//!
//! `src/cfg.zig` re-exports `print`, so `cfg.print` keeps working for AIR
...
/// Serialize a program back to the canonical AIR text form (air.md §9).
...
fn printFunc(out: *std.ArrayList(u8), allocator: std.mem.Allocator, types: []const TypeDecl, f: *const IrFunc) !void {
    ...
    // requires ids in text order (air.md §4.1). A match's test chain, for
...
fn printConst(out: *std.ArrayList(u8), allocator: std.mem.Allocator, c: ConstValue) !void {
    switch (c) {
        ...
        .float => |f| {
            // The AIR parser distinguishes floats from ints by the '.';
...
fn printInstr(out: *std.ArrayList(u8), allocator: std.mem.Allocator, types: []const TypeDecl, instr: *const Instr, block_index: *const std.AutoHashMap(*const BasicBlock, u32)) !void {
    ...
    switch (instr.op) {
        ...
        .neg, .abs, .clz, .popcount, .not_, .num_cast, .any_pack_copy, .any_pack_move, .any_unpack_copy, .any_unpack_move, .cleanup_arm, .copy, .borrow, .move_, .tail, .unpack_struct, .unpack_tuple, .split_list, .read_tag, .read_payload => |v| {
        ...
        .add, .sub, .mul, .div, .rem, .min, .max, .shl, .shr, .bitand, .bitor, .bitxor, .concat, .eq, .ne, .lt, .le, .gt, .ge => |bin| {
            try w(out, allocator, "{s} ", .{opText(instr.op)});
        ...
        .select => |s| {
            try w(out, allocator, "select ", .{});
            try printOperand(out, allocator, s.cond);
            try w(out, allocator, ", ", .{});
            try printOperand(out, allocator, s.a);
            try w(out, allocator, ", ", .{});
            try printOperand(out, allocator, s.b);
        },
        .load_member => |lm| {
        ...
        .phi => |phi| {
            ...
            // order (air.md §4.3), so a printed phi must list its inputs in
...
test "cfg round-trips call, syscall, and type_is" {
    var t = try parseText(
        ...
        \\        %4: int32 = syscall builtin#print, %1
        \\        ret %3
        \\    }
