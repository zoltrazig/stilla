...
pub const FuncInstance = struct {
    ...
    /// a deterministic, collision-free suffix for the instance's AIR
    /// function name (`{module}.{fn}.{id}`, air.md §11).
    id: u32,
};
...
/// item so one broken declaration does not hide its siblings' errors; the
/// consumer checks run only when annotation produced none (hard phase
/// ordering — their inputs would be noise).
pub const Checker = struct {
    allocator: std.mem.Allocator,
    graph: ?*moduleinfo.ModuleGraph = null,
    /// Every phase-2 diagnostic, in order (arena-owned).
    diags: std.ArrayList(moduleinfo.Diag) = .empty,
    /// The first semantic error (a view of `diags[0]`), for callers that
    /// only want one.
    ...
    /// The annotation arena; all phase-2 data (and the diagnostic
    /// messages) live here. The arena is a child of the compilation arena,
    ...
    pub fn check(self: *Checker, graph: *moduleinfo.ModuleGraph) CheckError!Annotation {
        ...
        for (graph.modules) |info| {
            ...
            for (program.items) |*item| switch (item.*) {
                .func_def => |*f| if (f.body == null and !info.bundle_origin) try self.annotation.host_bindings.put(self.alloc(), f, {}),
                else => {},
            };
        }

        for (graph.modules) |info| {
            annotate.annotateModule(self, info) catch |err| switch (err) {
                // Per-module continuation: a module whose annotation
                // aborted (e.g. an allocation-side failure or a
                // module-scope check) must not hide its siblings.
                error.Diagnostic => {},
                error.OutOfMemory => return error.OutOfMemory,
            };
        }
        // Hard phase ordering: the consumer checks consume the annotation
        // side tables, so they run only when annotation produced no
        // errors (their findings would be noise otherwise).
        if (self.diags.items.len == 0) {
            for (graph.modules) |info| {
                validate.validateModule(self, info) catch |err| switch (err) {
                    error.Diagnostic => {},
                    error.OutOfMemory => return error.OutOfMemory,
                };
            }
        ...
        if (self.diags.items.len > 0) return error.Diagnostic;
