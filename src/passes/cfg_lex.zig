//! IR text form lexer — ir.md §9 (the `cfg` text form).
//!
//! Turns the line-oriented IR text into the `Token` stream the parser
    ...
    pub fn tokenize(self: *Lexer, text: []const u8) LexError![]const Token {
        ...
        while (i < text.len) {
            ...
            const kind: TokKind = switch (ch) {
                ...
                '=' => .equals,
                '?' => .question,
                else => return self.lexFail(start, i, "unexpected character in IR text"),
            };
            i += 1;
