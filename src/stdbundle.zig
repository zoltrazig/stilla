...
//!
//! Resolution (`passes/module_load.zig`) instantiates these sources into the
//! module graph like ordinary imports; the phase-2 checker
//! (`passes/checker.zig`) flags their declaration-only members as host
//! bindings, mirroring the declarations here. The black-box
//! tests in `stdbundle_tests.zig` guarantee the bundle stays
