...
pub fn decodeScalar(heap: *VmHeap, pid: llir.PrimitiveId, cell: Value) !ScalarView {
    return switch (pid) {
        ...
        .i64 => .{ .i64 = @as(i64, @bitCast(cell)) },
        .u64 => .{ .u64 = cell },
        .float32 => .{ .float32 = ValueCodec.decodeFloat32(cell) orelse 0 },
        .f64 => .{ .f64 = ValueCodec.decodeFloat64(cell) },
        .bool => .{ .bool_ = cell != 0 },
...
pub fn formatView(view: ScalarView, buf: []u8) ![]const u8 {
    return switch (view) {
        ...
        .i64 => |v| std.fmt.bufPrint(buf, "{d}", .{v}),
        .u64 => |v| std.fmt.bufPrint(buf, "{d}", .{v}),
        .float32 => |v| std.fmt.bufPrint(buf, "{d}", .{v}),
        .f64 => |v| std.fmt.bufPrint(buf, "{d}", .{v}),
